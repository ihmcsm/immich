...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  AS $$
    BEGIN
      INSERT INTO person_audit ("personId", "groupId", "ownerId")
      SELECT "id", "groupId", "ownerId"
      FROM OLD;
      RETURN NULL;
    END
  $$;`.execute(db);
  ...
  await sql`ALTER TABLE "person" ADD "groupId" uuid;`.execute(db);
  ...
  await sql`UPDATE "person" SET "groupId" = "id";`.execute(db);
  await sql`ALTER TABLE "person" ALTER COLUMN "groupId" SET NOT NULL;`.execute(db);
  await sql`CREATE INDEX "person_groupId_idx" ON "person" ("groupId");`.execute(db);
  await sql`ALTER TABLE "person" ADD CONSTRAINT "person_groupId_fkey" FOREIGN KEY ("groupId") REFERENCES "person_group" ("id") ON UPDATE CASCADE ON DELETE CASCADE;`.execute(
    db,
  );

  await sql`ALTER TABLE "person_audit" ADD "groupId" uuid;`.execute(db);
  await sql`UPDATE "person_audit" SET "groupId" = "personId";`.execute(db);
  await sql`ALTER TABLE "person_audit" ALTER COLUMN "groupId" SET NOT NULL;`.execute(db);
  await sql`CREATE INDEX "person_audit_groupId_idx" ON "person_audit" ("groupId");`.execute(db);
  ...
  await sql`UPDATE "asset_face" SET "personGroupId" = "person"."groupId"
  FROM "person"
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  FROM "person"
  WHERE "person"."groupId" = "asset_face"."personId";`.execute(db);
  ...
  await sql`DROP INDEX "person_audit_groupId_idx";`.execute(db);
  await sql`ALTER TABLE "person_audit" DROP COLUMN "groupId";`.execute(db);
  await sql`ALTER TABLE "person" DROP CONSTRAINT "person_groupId_fkey";`.execute(db);
  await sql`DROP INDEX "person_groupId_idx";`.execute(db);
