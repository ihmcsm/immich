...
describe(PersonService.name, () => {
  ...
  describe('getAll', () => {
    it('should get all hidden and visible people with thumbnails', async () => {
      ...
      await expect(sut.getAll(auth, { withHidden: true, page: 1, size: 10 })).resolves.toEqual({
        ...
        hidden: 1,
        people: [
          expect.objectContaining({ id: person.groupId, isHidden: false }),
          expect.objectContaining({
            id: hiddenPerson.groupId,
            isHidden: true,
          }),
        ],
      });
    ...
    it('should get all visible people and favorites should be first in the array', async () => {
      ...
      await expect(sut.getAll(auth, { withHidden: false, page: 1, size: 10 })).resolves.toEqual({
        ...
        people: [
          expect.objectContaining({
            id: isFavorite.groupId,
            isFavorite: true,
          }),
          expect.objectContaining({ id: person.groupId, isFavorite: false }),
        ],
      });
      expect(mocks.person.getAllForUser).toHaveBeenCalledWith({ skip: 0, take: 10 }, auth.user.id, {
        withHidden: false,
  ...
  describe('getById', () => {
    it('should require person.read permission', async () => {
      ...
      await expect(sut.getById(auth, person.groupId)).rejects.toBeInstanceOf(BadRequestException);
      expect(mocks.access.person.checkOwnerAccess).toHaveBeenCalledWith(auth.user.id, new Set([person.groupId]));
    });
    ...
    it('should get a person by id', async () => {
      ...
      mocks.person.getByGroupId.mockResolvedValue(person);
      mocks.access.person.checkOwnerAccess.mockResolvedValue(new Set([person.groupId]));
      await expect(sut.getById(auth, person.groupId)).resolves.toEqual(expect.objectContaining({ id: person.groupId }));
      expect(mocks.person.getByGroupId).toHaveBeenCalledWith({ userId: auth.user.id, groupId: person.groupId });
      expect(mocks.access.person.checkOwnerAccess).toHaveBeenCalledWith(auth.user.id, new Set([person.groupId]));
    });
  });
  ...
  describe('getThumbnail', () => {
    it('should require person.read permission', async () => {
      ...
      mocks.person.getByGroupId.mockResolvedValue(person);
      await expect(sut.getThumbnail(auth, person.groupId)).rejects.toBeInstanceOf(BadRequestException);
    ...
    it('should throw an error when person has no thumbnail', async () => {
      ...
      mocks.person.getByGroupId.mockResolvedValue(person);
      mocks.access.person.checkOwnerAccess.mockResolvedValue(new Set([person.groupId]));
      await expect(sut.getThumbnail(auth, person.groupId)).rejects.toBeInstanceOf(NotFoundException);
      expect(mocks.storage.createReadStream).not.toHaveBeenCalled();
      expect(mocks.access.person.checkOwnerAccess).toHaveBeenCalledWith(auth.user.id, new Set([person.groupId]));
    });
    ...
    it('should serve the thumbnail', async () => {
      ...
      mocks.person.getByGroupId.mockResolvedValue(person);
      mocks.access.person.checkOwnerAccess.mockResolvedValue(new Set([person.groupId]));
      await expect(sut.getThumbnail(auth, person.groupId)).resolves.toEqual(
  ...
  describe('update', () => {
    it('should require person.write permission', async () => {
      ...
      mocks.person.getByGroupId.mockResolvedValue(person);
      await expect(sut.update(auth, person.groupId, { name: 'Person 1' })).rejects.toBeInstanceOf(BadRequestException);
    ...
    it("should update a person's name", async () => {
      ...
      await expect(sut.update(auth, person.groupId, { name: 'Person 1' })).resolves.toEqual(
        expect.objectContaining({ id: person.groupId, name: 'Person 1' }),
      );
    ...
    it("should update a person's date of birth", async () => {
      ...
      await expect(sut.update(auth, person.groupId, { birthDate: '1976-06-30' })).resolves.toEqual({
        id: person.groupId,
        name: person.name,
    ...
    it('should update a person visibility', async () => {
      ...
      await expect(sut.update(auth, person.groupId, { isHidden: true })).resolves.toEqual(
    ...
    it('should update a person favorite status', async () => {
      ...
      await expect(sut.update(auth, person.groupId, { isFavorite: true })).resolves.toEqual(
    ...
    it("should update a person's thumbnailPath", async () => {
      ...
      await expect(sut.update(auth, person.groupId, { featureFaceAssetId: face.assetId })).resolves.toEqual(
        expect.objectContaining({ id: person.groupId }),
      );
    ...
    it('should throw an error when the face feature assetId is invalid', async () => {
      ...
      await expect(sut.update(auth, person.groupId, { featureFaceAssetId: '-1' })).rejects.toThrow(BadRequestException);
      expect(mocks.person.update).not.toHaveBeenCalled();
      expect(mocks.access.person.checkOwnerAccess).toHaveBeenCalledWith(auth.user.id, new Set([person.groupId]));
    });
  });
  ...
  describe('reassignFaces', () => {
    ...
    it('should reassign a face', async () => {
      ...
      await expect(
        sut.reassignFaces(auth, person.groupId, {
          data: [{ personId: person.groupId, assetId: face.assetId }],
        }),
  ...
  describe('createFace', () => {
    it('should create a manual face and initialize the person feature photo creation', async () => {
      ...
      expect(mocks.person.getRandomFace).toHaveBeenCalledWith(person.groupId);
  ...
  describe('reassignFacesById', () => {
    it('should create a new person', async () => {
      ...
      mocks.person.reassignFace.mockResolvedValue(1);
      mocks.person.getByGroupId.mockResolvedValue(person);
      await expect(sut.reassignFacesById(AuthFactory.create(), person.groupId, { id: face.id })).resolves.toEqual({
  ...
  describe('createPerson', () => {
    it('should create a new person in a new group', async () => {
      ...
      mocks.person.create.mockResolvedValue(PersonFactory.create({ groupId: group.id }));
      await expect(sut.create(auth, {})).resolves.toBeDefined();

      expect(mocks.person.createGroup).toHaveBeenCalledWith(auth.user.id);
      expect(mocks.person.create).toHaveBeenCalledWith({ ownerId: auth.user.id, groupId: group.id });
    });
  });
  ...
  describe('handlePersonCleanup', () => {
    it('should delete people without faces', async () => {
      ...
      expect(mocks.person.deleteGroups).toHaveBeenCalledWith([person.groupId]);
  ...
  describe('handleRecognizeFaces', () => {
    ...
    it('should match existing person', async () => {
      ...
      expect(mocks.person.reassignFaces).toHaveBeenCalledWith({
        ...
        newPersonGroupId: primaryFace.person!.groupId,
      });
      expect(mocks.person.reassignFaces).toHaveBeenCalledWith({
    ...
    it('should match existing person if their birth date is unknown', async () => {
      ...
      expect(mocks.person.reassignFaces).toHaveBeenCalledWith({
        ...
        newPersonGroupId: face.person!.groupId,
      });
      expect(mocks.person.reassignFaces).toHaveBeenCalledWith({
        faceIds: expect.not.arrayContaining([face.id]),
        newPersonGroupId: face.person!.groupId,
    ...
    it('should match existing person if their birth date is before file creation', async () => {
      ...
      expect(mocks.person.reassignFaces).toHaveBeenCalledWith({
        faceIds: expect.arrayContaining([noPerson.id]),
        newPersonGroupId: faceWithBirthDate.person!.groupId,
      });
      expect(mocks.person.reassignFaces).toHaveBeenCalledWith({
        faceIds: expect.not.arrayContaining([face.id]),
        newPersonGroupId: faceWithBirthDate.person!.groupId,
    ...
    it('should create a new person if the face is a core point with no person', async () => {
      ...
      mocks.search.searchFaces.mockResolvedValue(faces);
      mocks.person.getFaceForFacialRecognitionJob.mockResolvedValue(getForFacialRecognitionJob(noPerson1, asset));
      mocks.person.createGroup.mockResolvedValue(PersonGroupFactory.create({ id: person.groupId }));
      ...
      expect(mocks.person.reassignFaces).toHaveBeenCalledWith({
        ...
        newPersonGroupId: person.groupId,
      });
    });
    ...
    it('should create a person in the matched group when the match belongs to another user', async () => {
      ...
      const person = PersonFactory.create({ ownerId: asset.ownerId, groupId: otherOwnerFace.person!.groupId });
      ...
      expect(mocks.person.create).toHaveBeenCalledWith({
        ...
        groupId: otherOwnerFace.person!.groupId,
      });
      expect(mocks.person.reassignFaces).toHaveBeenCalledWith({
        faceIds: [noPerson.id],
        newPersonGroupId: otherOwnerFace.person!.groupId,
      });
    });
  ...
  describe('mergePerson', () => {
    it('should require person.write and person.merge permission', async () => {
      ...
      await expect(sut.mergePerson(auth, person.groupId, { ids: [mergePerson.id] })).rejects.toBeInstanceOf(
        BadRequestException,
      );
    ...
    it('should merge two people without smart merge', async () => {
      ...
      mocks.access.person.checkOwnerAccess.mockResolvedValueOnce(new Set([person.groupId]));
      mocks.access.person.checkOwnerAccess.mockResolvedValueOnce(new Set([mergePerson.groupId]));

      await expect(sut.mergePerson(auth, person.groupId, { ids: [mergePerson.groupId] })).resolves.toEqual([
        { id: mergePerson.groupId, success: true },
      ]);

      expect(mocks.person.reassignFaces).toHaveBeenCalledWith({
        newPersonGroupId: person.groupId,
        oldPersonGroupId: mergePerson.groupId,
      });
    ...
    it('should handle invalid merge ids', async () => {
      ...
      await expect(sut.mergePerson(auth, person.groupId, { ids: ['unknown'] })).resolves.toEqual([
    ...
    it('should handle an error reassigning faces', async () => {
      ...
      await expect(sut.mergePerson(auth, person.groupId, { ids: [mergePerson.id] })).resolves.toEqual([
  ...
  describe('getStatistics', () => {
    it('should get correct number of person', async () => {
      ...
      mocks.access.person.checkOwnerAccess.mockResolvedValue(new Set([person.groupId]));
      await expect(sut.getStatistics(auth, person.groupId)).resolves.toEqual({ assets: 3 });
      expect(mocks.access.person.checkOwnerAccess).toHaveBeenCalledWith(auth.user.id, new Set([person.groupId]));
    });
    ...
    it('should require person.read permission', async () => {
      ...
      mocks.person.getByGroupId.mockResolvedValue(person);
      await expect(sut.getStatistics(auth, person.groupId)).rejects.toBeInstanceOf(BadRequestException);
      expect(mocks.access.person.checkOwnerAccess).toHaveBeenCalledWith(auth.user.id, new Set([person.groupId]));
    });
  });
