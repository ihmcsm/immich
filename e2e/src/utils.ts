...
export const utils = {
  ...
  createFace: async ({ assetId, personId }: { assetId: string; personId: string }) => {
    ...
    await client.query(
      'INSERT INTO asset_face ("assetId", "personId") SELECT $1, "id" FROM "person" WHERE "groupId" = $2',
      [assetId, personId],
    );
  },
  ...
  setPersonThumbnail: async (personId: string) => {
    ...
    await client.query(`UPDATE "person" set "thumbnailPath" = '/my/awesome/thumbnail.jpg' where "groupId" = $1`, [
      personId,
    ]);
  },
